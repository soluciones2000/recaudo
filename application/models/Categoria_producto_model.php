...
class Categoria_producto_model extends CI_Model {
	...
	var $table = 'categoriaproducto';
	var $column_order = array('nombreCategoria', null); //set column field database for datatable orderable
	var $column_search = array('nombreCategoria'); //set column field database for datatable searchable 
	var $order = array('nombreCategoria' => 'asc'); // default order 
	var $id = "id";
	...
	private function _get_datatables_query()
	{
		...
		if($this->input->post('NombreCategoria_ajaxfiltro'))
		{
			$this->db->like('NombreCategoria', $this->input->post('NombreCategoria_ajaxfiltro', TRUE));
		}
	...
	{
		$this->db->from($this->table);
		$this->db->where(array("NombreCategoria" => $data["NombreCategoria"]));
		$query = $this->db->get();
		if($query->num_rows() > 0)
		{
	...
	public function update($where, $data)
	{
		$query = $this->db->from($this->table) // esta sentencia verifique q no existan repetidos y permite guardar la misma info si no se modifica
				->group_start()
					...
					->group_start()
						->where("NombreCategoria",$data["NombreCategoria"]) // campo unico
					->group_end()
				->group_end()
			->get();
