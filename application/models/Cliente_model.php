...
class Cliente_model extends CI_Model {
	...
	var $table = 'cliente';
	var $column_order = array('NombreCliente','IdFiscal',"FechaRegistro", null); //set column field database for datatable orderable
	var $column_search = array('NombreCliente','IdFiscal',"FechaRegistro"); //set column field database for datatable searchable 
	var $order = array('NombreCliente' => 'asc'); // default order 
	var $id = "id";
	...
	private function _get_datatables_query()
	{
		...
		if($this->input->post('NombreCliente_ajaxfiltro'))
		{
			$this->db->like('NombreCliente', $this->input->post('NombreCliente_ajaxfiltro', TRUE));
		}
	...
	public function update($where, $data)
	{
		...
		if($query->num_rows() > 0)// si existe el id fiscal de un Cliente en la bd y se quiere modificar a ese mismo id existente
		{
			return array("status" => "error");
		}
		else
