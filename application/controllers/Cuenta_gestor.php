...
class Cuenta_gestor extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('Cuenta_gestor_model',"model");
	...
	public function index()
	{
		if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
		{
			...
			$data['titulo'] = "Cuenta de Gestor/Cliente";//titulo de la pagina

			$this->_render_page("cuenta_gestor_view", $data);
		}
		else
		{	
			redirect("Login"); // redirige al controlador login si se intenta entrar a este controlador sin tener una session activa
		...
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1 || $this->sesion == 2) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				$data = array(
						...
						"idGestor" => $this->input->post("idGestor_form", TRUE),
				...
				$this->seguridad->registrar_accion("Se agrega la cuenta de gestor/cliente con el id ".$insert["insert_id"]);
				exit();
			}
			else
			{	
	...
	public function ajax_update()// modifica un registro
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				//registro de acciones
				$this->seguridad->registrar_accion("Se modifica la cuenta de gestor/cliente con el id ".$this->input->post('id', TRUE));
	...
	public function ajax_delete($id = FALSE)//borrar un registro en base a un id
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			...
			{	
				//registro de acciones
				$this->seguridad->registrar_accion("Se Borro la cuenta de gestor/cliente con el id ".$id);
    ...
    public function reporte()//realiza un reporte impreso en pdf que se muestra en la pagina
    {
    	if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
	    	if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
		        ...
		        $this->html2pdf->html(utf8_decode($this->load->view('reportes_pdf/Cuenta_gestor_report_pdf', $data, true)));
	...
	public function cargar_gestor()
	{
		if ($this->input->is_ajax_request())
		{
			...
			{
				$data = array();
				$data["info"] = $this->model->get_list_filtro("gestor","NombreGestor");
