...
class Cuenta_proveedor extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('Cuenta_proveedor_model',"model");
	...
	public function index()
	{
		if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
		{
			...
			$data['titulo'] = "Cuenta de proveedor/Cliente";//titulo de la pagina

			$this->_render_page("cuenta_proveedor_view", $data);
		}
		else
		{	
			redirect("Login"); // redirige al controlador login si se intenta entrar a este controlador sin tener una session activa
		...
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				foreach ($this->data['users'] as $rows)
				{
					...
					$row = array();
					$row[] = $no;
					$row[] = $rows->Nombre_Cuenta;
					$row[] = $rows->SaldoCuenta;
					$row[] = '
						<div class="dropdown">
	...
	public function ajax_add()//agrega un nuevo registro
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1 || $this->sesion == 2) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				$data = array(
						"idCliente" => $this->input->post("idCliente_form", TRUE),
						"idProveedor" => $this->input->post("idProveedor_form", TRUE),
						"Nombre_Cuenta" => $this->input->post("Nombre_Cuenta_form", TRUE),
						"SaldoCuenta" => $this->input->post("SaldoCuenta_form", TRUE),
					);

				$insert = $this->model->save($data);
				echo json_encode($insert);
				//registro de acciones
				$this->seguridad->registrar_accion("Se agrega la cuenta de proveedor/cliente con el id ".$insert["insert_id"]);
				exit();
			}
			else
			{	
	...
	public function ajax_update()// modifica un registro
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				//registro de acciones
				$this->seguridad->registrar_accion("Se modifica la cuenta de proveedor/cliente con el id ".$this->input->post('id', TRUE));
	...
	public function ajax_delete($id = FALSE)//borrar un registro en base a un id
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			...
			{	
				//registro de acciones
				$this->seguridad->registrar_accion("Se Borro la cuenta de proveedor/cliente con el id ".$id);
    ...
    public function reporte()//realiza un reporte impreso en pdf que se muestra en la pagina
    {
    	if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
	    	if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
		        ...
		        //registro de acciones
				$this->seguridad->registrar_accion("Se realizo un reporte de las cuentas de proveedor/cliente");

		        $row_empresa = $this->pdf_model->get_empresa($this->input->post("id_empresa", TRUE));

		        $data = array(
		            "logo" => "./files/img/logos_empresas/".$row_empresa->archivo,
		            'title' => "Cuentas de proveedor/cliente",
		            "header" => utf8_encode($row_empresa->nombre_empresa),
		            'datos' => $this->pdf_model->get_reporte_Categoria_producto(),
		        ...
		        $this->html2pdf->html(utf8_decode($this->load->view('reportes_pdf/Cuenta_proveedor_report_pdf', $data, true)));
	...
	public function cargar_cliente()
	{
		if ($this->input->is_ajax_request())
		{
			...
			{
				$data = array();
				$data["info"] = $this->model->get_list_filtro("cliente","NombreCliente");
	...
	public function cargar_proveedor()
	{
		if ($this->input->is_ajax_request())
		{
			...
			{
				$data = array();
				$data["info"] = $this->model->get_list_filtro("proveedor","NombreProveedor");
    ...
    private function _validate() // valida el envio de los campos si estan vacios o no
	{
		...
		if($this->input->post('Nombre_Cuenta_form') == '')
		{
			$data['inputerror'][] = 'Nombre_Cuenta_form';
			$data['error_string'][] = 'Nombre de la cuenta es requerido';
			$data['status'] = FALSE;
		}
		if($this->input->post('SaldoCuenta_form') == '')
		{
			$data['inputerror'][] = 'SaldoCuenta_form';
			$data['error_string'][] = 'Saldo de la cuenta es requerido';
			$data['status'] = FALSE;
		}
