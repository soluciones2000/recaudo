        ...
        <div class="card">
            ...
            <div class="card-body">
            	<form id="form-filter" class="form-horizontal">
                    <div class="row">
                        <div class="col-md-6">
                            <p class="lead">Nombre del Producto</p>
                        </div>
                        <div class="col-md-6">
                            <input type="text" class="form-control" id="NombreProducto_filtro">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
...
<script type="text/javascript">
        ...
        table = $('#table').DataTable({ 
            ...
            "ajax": {
    			...
    			{    // are send to the server side
    				data.NombreProducto_ajaxfiltro = $('#NombreProducto_filtro').val(); 
    ...
    {
        $.ajax({
            url: "<?php echo base_url('Producto/get_list_empresas')?>",
            dataType:"JSON",
            type: "POST",
        ...
        $.ajax({
            url: "<?php echo base_url('Producto/get_list_proveedor')?>",
            dataType:"JSON",
            type: "POST",
            success: function(data)
            {
                if(data.status != false)
                {
                    ...
                    $('[name="id_proveedor_form"]').html(output.join(''));//agrega el array y lo muestra en el html
                }
                else
                {
                    alert("No posee el nivel de seguridad requerido");
    ...
    function reporte_pdf()
    {
        var parametros = {// are send to the server side
            ...
            NombreProducto_ajaxreporte : $('#NombreProducto_filtro').val(),
            FechaRegistro_ajaxreporte : $('#FechaRegistro_filtro').val(),
        };

        $.ajax({
            url: '<?php echo base_url("Producto/reporte");?>',
            type: "POST",
            dataType: 'JSON',
            data: parametros,
            success: function(data)
    ...
    function detalles_ajax(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                {
                    $('[name="id"]').val(data.id);
                    $('[name="NombreProducto_form"]').val(data.NombreProducto);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo base_url('Producto/ajax_add')?>";
        } else {
            url = "<?php echo base_url('Producto/ajax_update')?>";
        }
    ...
    function borrar_ajax(id)
    {
        if(confirm('Esta seguro de querer borrar esta información?'))
        {
            ...
            $.ajax({
                url : "<?php echo base_url('Producto/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <input name="NombreProducto_form" placeholder="Nombre del Producto" class="form-control" type="text" maxlength="50">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <select name="id_proveedor_form" class="form-control">
                                </select>
                                <span class="help-block"></span>
                            </div>
                        </div>
