        ...
        <div class="card">
            ...
            <div class="card-body">
            	<form id="form-filter" class="form-horizontal">
                    <div class="row">
                        <div class="col-md-6">
                            <p class="lead">Nombre de la categoria</p>
                        </div>
                        <div class="col-md-6">
                            <input type="text" class="form-control" id="NombreFormaPago_filtro">
                        </div>
                    </div>
                    <div class="row">
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        cargar_proveedor();// carga el proveedor al cargar la pagina por primera vez
        ...
        table = $('#table').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo base_url('Forma_pago/ajax_list')?>",
                "type": "POST",
                "data": function (data)
    			{    // are send to the server side
    				data.NombreFormaPago_ajaxfiltro = $('#NombreFormaPago_filtro').val(); 
                }
            },
    ...
    {
        $.ajax({
            url: "<?php echo base_url('Forma_pago/get_list_empresas')?>",
            dataType:"JSON",
            type: "POST",
    ...
    function reporte_pdf()
    {
        var parametros = {// are send to the server side
            ...
            NombreFormaPago_ajaxreporte : $('#NombreFormaPago_filtro').val(),
        };

        $.ajax({
            url: '<?php echo base_url("Forma_pago/reporte");?>',
            type: "POST",
            dataType: 'JSON',
            data: parametros,
            success: function(data)
    ...
    {
        $.ajax({
            url: "<?php echo base_url('Forma_pago/cargar_gestor')?>",
            dataType:"JSON",
            type: "POST",
            success: function(data)
            {
    ...
    function cargar_proveedor(id = false)
    {
        $.ajax({
            url: "<?php echo base_url('Forma_pago/cargar_proveedor')?>",
            dataType:"JSON",
            type: "POST",
            success: function(data)
            {
                if(data.status != false)
                {
                    if (id != false)
                    {
                        ...
                        $('[name="idProveedor_form"]').html(output.join(''));//agrega el array y lo muestra en el html
                    }
                    else
                    {
                        var output = [];//array en javascript
    ...
    function detalles_ajax(id)
    {
        ...
        $.ajax({
            url : "<?php echo base_url('Forma_pago/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                ...
                {
                    $('[name="id"]').val(data.id);
                    $('[name="idProveedor_form"]').val(data.idProveedor);
                    $('[name="idGestor_form"]').val(data.idGestor);
                    $('[name="NombreFormaPago_form"]').val(data.NombreFormaPago);

                    cargar_gestor(data.idGestor);
                    cargar_proveedor(data.idProveedor);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo base_url('Forma_pago/ajax_add')?>";
        } else {
            url = "<?php echo base_url('Forma_pago/ajax_update')?>";
        }
    ...
    function borrar_ajax(id)
    {
        if(confirm('Esta seguro de querer borrar esta información?'))
        {
            ...
            $.ajax({
                url : "<?php echo base_url('Forma_pago/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <select name="idProveedor_form" class="form-control">
                                </select>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <p class="lead">Nombre de la forma de pago</p>
                            </div>
                            <div class="col-md-6">
                                <input name="NombreFormaPago_form" placeholder="Nombre de la forma de pago" class="form-control" type="text" maxlength="50">
                                <span class="help-block"></span>
                            </div>
                        </div>
                    </div>
