...
class Proveedor extends CI_Controller {
	...
	public function index()
	{
		if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
		{
			...
			$data['titulo'] = "Proveedor";//titulo de la pagina

			$this->_render_page("proveedor_view", $data);
		}
		else
		{	
			redirect("Login"); // redirige al controlador login si se intenta entrar a este controlador sin tener una session activa
					...
					$row[] = '
						<div class="dropdown">
						  	...
						  	<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
								<li><a class="btn btn-sm btn-primary btn-block" style="color: black;" href="javascript:void(0)" title="Editar" onclick="editar_ajax('."'".$rows->idProveedor."'".')"><i class="glyphicon glyphicon-pencil"></i> Editar</a></li>
								<li><a class="btn btn-sm btn-info btn-block" style="color: black;" href="javascript:void(0)" title="Detalles" onclick="detalles_ajax('."'".$rows->idProveedor."'".')"><i class="glyphicon glyphicon-list-alt"></i> Detalles</a></li>
								<li><a class="btn btn-sm btn-danger btn-block" style="color: black;" href="javascript:void(0)" title="Borrar" onclick="borrar_ajax('."'".$rows->idProveedor."'".')"><i class="glyphicon glyphicon-trash"></i> Borrar</a></li>
						  	</ul>
						</div>
						';
	...
	public function ajax_add()//agrega un nuevo registro
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1 || $this->sesion == 2) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				$data = array(
						'NombreProveedor' => $this->input->post('NombreProveedor_form', TRUE),
						"IdFiscal" => $this->input->post("IdFiscal_form", TRUE),
						"FechaRegistro" => $this->input->post("FechaRegistro_form", TRUE),
					);

				$insert = $this->model->save($data);
				echo json_encode($insert);
				//registro de acciones
				$this->seguridad->registrar_accion("Se agrega el proveedor con el id ".$insert["insert_id"]);
				exit();
			}
			else
			{	
	...
	public function ajax_update()// modifica un registro
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			if($this->sesion == 1) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
				...
				//registro de acciones
				$this->seguridad->registrar_accion("Se modifica el proveedor con el id ".$this->input->post('id', TRUE));
				$update = $this->model->update(array('idProveedor' => $this->input->post('id', TRUE)), $data);
	...
	public function ajax_delete($id = FALSE)//borrar un registro en base a un id
	{
		if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
			...
			{	
				//registro de acciones
				$this->seguridad->registrar_accion("Se Borro el proveedor con el id ".$id);
    ...
    public function reporte()//realiza un reporte impreso en pdf que se muestra en la pagina
    {
    	if ($this->input->is_ajax_request()) //permite validar si la ejecucion del metodo es por ajax
		{
	    	if($this->sesion == 1 || $this->sesion == 2 || $this->sesion == 3) //valida el nivel de acceso del usuario y permite ejecutar las funciones
			{
		        ...
		        //registro de acciones
				$this->seguridad->registrar_accion("Se realizo un reporte de los proveedores");
		        ...
		        $data = array(
		            ...
		            'datos' => $this->pdf_model->get_reporte_proveedor(),
		        ...
		        $this->html2pdf->html(utf8_decode($this->load->view('reportes_pdf/proveedor_report_pdf', $data, true)));
    ...
    private function _validate() // valida el envio de los campos si estan vacios o no
	{
		...
		if($this->input->post('NombreProveedor_form') == '')
		{
			$data['inputerror'][] = 'NombreProveedor_form';
			$data['error_string'][] = 'Nombre del proveedor es requerido';
			$data['status'] = FALSE;
		}
		if($this->input->post('IdFiscal_form') == '')
		{
			$data['inputerror'][] = 'IdFiscal_form';
			$data['error_string'][] = 'ID Fiscal es requerido';
			$data['status'] = FALSE;
		}
		if($this->input->post('FechaRegistro_form') == '')
		{
			$data['inputerror'][] = 'FechaRegistro_form';
			$data['error_string'][] = 'Fecha de registro es requerido';
			$data['status'] = FALSE;
		}
